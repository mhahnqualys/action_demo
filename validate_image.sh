...
#echo -n "$Q_USERNAME" | wc
#echo -n "$Q_PASSWORD" | wc

#echo -n "$Q_USERNAME" | shasum -a 256
#echo -n "$Q_PASSWORD" | shasum -a 256
...
get_token() {
    ...
    #echo "$CURL" -k -X POST "${QUALYS_API_SERVER}"'/auth' --data-urlencode 'username='"${Q_USERNAME}" --data-urlencode 'password='"${Q_PASSWORD}" --data-urlencode 'token=true' --data-urlencode 'permissions=true' -s
	TOKEN=$("$CURL" -k -X POST "${QUALYS_API_SERVER}"'/auth' --data-urlencode 'username='"${Q_USERNAME}" --data-urlencode 'password='"${Q_PASSWORD}" --data-urlencode 'token=true' --data-urlencode 'permissions=true' -s)
	echo "$TOKEN"
}
...
get_result
wait_period=0
while [ "${HTTP_CODE}" -ne "200" ] || [ "${VULNS_AVAILABLE}" != true ] && [[ "$wait_period" -lt "$TIMEOUT" ]]
do
    time_left=$((TIMEOUT-wait_period))
	echo "Retrying after 10 seconds... ($time_left left)"
	sleep 10s
